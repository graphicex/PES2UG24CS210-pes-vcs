...
int commit_serialize(const Commit *commit, void **data_out, size_t *len_out) {
    ...
    n += snprintf(buf + n, sizeof(buf) - n,
                  ...
                  commit->author, commit->timestamp, commit->message);
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    ObjectID parent_id;
    if (head_read(&parent_id) == 0) { c.has_parent = 1; c.parent = parent_id; }
    else { c.has_parent = 0; }
    ...
    size_t dlen = strlen(display_msg);
    if (dlen > 0 && display_msg[dlen - 1] == '\n') display_msg[dlen - 1] = '\0';
    printf("[main %.7s] %s\n", hex, display_msg);
